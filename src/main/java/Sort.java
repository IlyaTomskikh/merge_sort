...
public class Sort {
    public static void main(String[] args) {
        ...
        String mode = "-a";
        String dest;
        if (args[0].equals("-a") || args[0].equals("-d")) { //а если здесь нету args[0] или args[1]? Обязательно нужно проверить, что в массиве есть эти элементы)
            mode = args[0];
            ...
            //данная ошибка (текст) не полностью отражет кейс, т.к. сюда попадают все случаи, которые не вписались в условия выше
            //то есть и про args[1] речь идет
            //кроме того, рекомендую в ошибку выводить не args[N], а название параметра
            //кстати, можешь завести команду help (это как один из вариантов args можно считывать, и есть запрошен help - печать инструкции на экран (в терминале введи java help - та же суть)
    ...
    //два замечания по коду ниже (позднее посмотрю подробнее сами реализации методов)
    //1. Методы очень большие. Это НЕ КРИТИЧНОЕ замечание, но если какой-то код можешь вытащить в отдельные функции и это имеет смысл - делай. Особенно если есть повторяющиеся блоки кода
    //2. Я не вдавалась пока в нюансы реализации, но на первый взгляд мне не ясно соблюдается ли условие из задания касательно больших файлов, которые не помещаются целиком в ОЗУ?
    //ты все считываешь в память и там же мержишь?
    //ПС. Пройдись по заданию и посмотри внимательно насколько твоя реализация удовлетворяет всем условиям
    ...
    public static boolean mergeSortString(String[] paths, String destination, String mode) {
        ...
        //так, кажется тут два одинаковых блока ниже?
        try {
            fileWriter.close();
        } catch (IOException e) {
            System.out.println("ERROR InputOutput: couldn't close a file");
